...
export function WeeklyChallenge({ onUnlocked, scrollRef }: WeeklyChallengeProps) {
  ...
  return (
    ...
    >
      {/* Header banner */}
      <div className="bg-gradient-to-r from-[#111214] via-[#1e2024] to-[#3a3d44] p-4 sm:p-5 border-b border-[#d4881f]/40">
          ...
          {options.map((option, i) => {
            ...
            return (
              <button
                ...
                } ${isSelected ? "ring-1 ring-[#d4881f]/40" : ""}`}
              >
                {hasVoted && (
                  <motion.div
                    initial={{ width: 0 }}
                ...
                <div className="relative flex items-center justify-between px-4 py-3.5">
                  <div className="flex items-center gap-3">
                    {!hasVoted && (
                      <div className="w-5 h-5 rounded-full border-2 border-[#d4881f]/60 flex-shrink-0" />
                    )}
                    {hasVoted && isSelected && (
                      <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="w-5 h-5 rounded-full bg-[#d4881f] flex-shrink-0 flex items-center justify-center"
                      >
